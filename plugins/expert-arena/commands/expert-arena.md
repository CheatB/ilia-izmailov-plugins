...
2. **РАЗНЫЕ точки зрения** — КРИТИЧЕСКИ ВАЖНО. Не набирай единомышленников. Ищи людей, которые РЕАЛЬНО спорили бы друг с другом. Примеры:
   ...
   - Python архитектура: Raymond Hettinger (pythonic) vs Bob Martin (Clean Code) vs Armin Ronacher (Flask/pragматик)
   - Telegram боты: Aiogram patterns vs python-telegram-bot vs Telegraf (Node.js)
   - Async Python: David Beazley (coroutines/async deep) vs Łukasz Langa (typing/mypy) vs Andrew Svetlov (aiohttp/практик)
   - Базы данных: Martin Kleppmann (DDIA) vs Hector Garcia-Molina (distributed) vs Rick Branson (Postgres-практик)
...
3. **РАЗНЫЕ углы зрения:**
   ...
   - Python vs Node.js vs Go (если вопрос о стеке)
...
СТЕК: Проверь requirements.txt, pyproject.toml, package.json — определи стек."

Researcher 2: "Найди актуальные best practices и мнения экспертов.
КОНТЕКСТ ВОПРОСА: [вопрос]
ФОКУС: Через WebSearch — свежие статьи 2025-2026, обсуждения, сравнения подходов."

Researcher 3: "Проанализируй ограничения и техдолг.
КОНТЕКСТ ВОПРОСА: [вопрос]
ФОКУС: Существующие решения, зависимости, потенциальные конфликты.
ПРОВЕРЬ: Docker-конфиг, миграции БД, CI/CD пайплайны."
```
...
[Из Researcher 1 — стек, архитектура, Docker-сервисы]

### Актуальные данные и практики
[Из Researcher 2]

### Ограничения и прецеденты
[Из Researcher 3 — техдолг, миграции, CI/CD]
```
...
**Имена (slug):** строчные латинские с дефисами: `martin-fowler`, `dhh`, `nassim-taleb`, `raymond-hettinger`
...
| Эксперт сменил позицию | "**Hettinger сдвинулся!** Отказался от pure-pythonic после аргумента Martin про maintainability. Теперь предлагает pragmatic-clean" |
...
| Эксперт упорствует против большинства | "**Ronacher не сдаётся** — нашёл structural flaw, который async не покрывает: unknown-unknowns" |
| Неожиданный аргумент | "Интересный ход: Beazley включил идею Langa (typing) в свой counter-proposal по async" |
